...
from utils.utils import sparse_to_dense_channel, sparse_to_dense_torch
from network.modules import ConvGRU
...
class GRUFusion(nn.Module):
    ...
    def __init__(self, cfg, ch_in = None, direct_substitute = False):
        ...
        self.n_scles = len(cfg.THRESHOLDS) - 1
        ...
        if direct_substitute:
            ...
            for i, ch in enumerate(ch_in):
                self.fusion_nets.append((ConvGRU(hidden_dim = ch, input_dim = ch, pres = 1, vres = self.cfg.VOXEL_SIZE * 2 ** (self.n_scles - i))))
    ...
    def conver2dense(self, current_coords, current_values, coords_target_global, tsdf_target, relative_origin, scale):
    ...
    def forward(self, coords, values_in, inputs, scale=2, outputs=None, save_mesh=False):
        ...
        for i in range(batch_size):
            ...
            if self.scene_name[scale] is not None or scene != self.scene_name[scale]:
            ...
            # relative origin in glboal volume
            ...
            updated_coords, current_volume, global_volume, target_volume, valid, valid_target = self.conver2dense(coords_b, values, coords_target, tsdf_target, relative_origin, scale)
            
            # dense to sprase : get features using new feature coordinates
